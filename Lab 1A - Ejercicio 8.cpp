// Laboratorio 1A - Ejercicio #8 / Jose Pablo Agüero Mora (2021126372)
...
void main() // Función main, inicia y termina dentro de las llaves, el programa inicia su ejecución en esta función
{
	...
	for (int i = 0; i < 100; i++) { // Se genera un ciclo for que se inicializa en 0 y debe llegar a 99
		ArregloDeCienEnteros[i] = i; // Se recorre el arreglo según el contador del for y se le asigna el valor de la posición del arreglo
...
/* El programa genera un arreglo de enteros (int) el cual recorre mediante un ciclo for y define sus valores mediante un contador */